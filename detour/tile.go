...
func (s *navMeshTileHeader) Serialize(dst []byte) {
	...
	little.PutUint64(dst[off:], uint64(s.TileRef))
	little.PutUint32(dst[off+8:], uint32(s.DataSize))
}
...
// Reads structured binary data from src, keeping the change in read offset divisible by 4
//   src = the raw data
//   dest = a pointer to the structured data array
// Returns
//   next = src[the size of this data:]
func readAlign4Binary(src []byte, dest interface{}) (next []byte) {
	bSize := binary.Size(dest)
	size := align4(bSize)
	slice := src[:size]
...
func (s *MeshTile) unserialize(hdr *MeshHeader, src []byte) Status {
	s.Verts = make([]float32, 3*hdr.VertCount)
	src = readAlign4Binary(src, &s.Verts)

	s.Polys = make([]Poly, hdr.PolyCount)
	src = readAlign4Binary(src, &s.Polys)

	s.Links = make([]Link, hdr.MaxLinkCount)
	src = readAlign4Binary(src, &s.Links)

	s.DetailMeshes = make([]PolyDetail, hdr.DetailMeshCount)
	src = readAlign4Binary(src, &s.DetailMeshes)

	s.DetailVerts = make([]float32, 3*hdr.DetailVertCount)
	src = readAlign4Binary(src, &s.DetailVerts)

	s.DetailTris = make([]uint8, 4*hdr.DetailTriCount)
	src = readAlign4Binary(src, &s.DetailTris)

	s.BvTree = make([]BvNode, hdr.BvNodeCount)
	src = readAlign4Binary(src, &s.BvTree)

	s.OffMeshCons = make([]OffMeshConnection, hdr.OffMeshConCount)
	src = readAlign4Binary(src, &s.OffMeshCons)

	for i := range s.BvTree {
		t := &s.BvTree[i]
		if int(t.I) >= len(s.Polys) {
